...
export default function FileList({ files, onUpdateFile }: FileListProps) {
  ...
  const getFilteredFiles = useCallback(() => {
    ...
    if (!activeTag && exactTagMatch) {
      filteredFiles = filteredFiles.filter((file) =>
        file.tags.some(
          (tag) => tag.toLowerCase() === exactTagMatch.toLowerCase()
        )
      );
    }
  ...
  const saveChanges = useCallback(
    (fileId: string, name: string, category: string, tags: string[]) => {
                ...
                <div key={category}>
                  <h3 className="text-sm font-medium mb-3 px-1 text-gray-700 flex items-center">
                    ...
                    <span className="ml-2 text-gray-400 text-xs">
                      ({filteredFilesByCategory[category].length}{" "}
                      {filteredFilesByCategory[category].length === 1
                        ? "dokument"
                        : "dokumenter"}
                      )
                    </span>
